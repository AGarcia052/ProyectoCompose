...
class ListaUsuariosViewModel:ViewModel() {
    ...
    fun downloadImage(email: String) {
        var fileUrl:String? = ""
        db.collection(Colecciones.usuarios).document(email).get().addOnSuccessListener { result ->
            val datos = result.data
            datos?.let {
                fileUrl = datos["foto"] as String
            }
            if (fileUrl == "" || fileUrl == null){
                _imageUri.value = Uri.parse("android.resource://com.example.restauranteapp/drawable/pfp")
            }else{
                Log.d(TAG, "downloadImage: $fileUrl")
                _imageUri.value = Uri.parse(fileUrl)
            }
        }
    }
    ...
    fun cambiarEstadoUsuario(activo: Boolean){
        val usuario = _usuarioAEditar.value!!.copy(activo = activo)
